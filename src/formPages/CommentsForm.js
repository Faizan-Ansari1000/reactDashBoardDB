...
export default function CommentsForm() {
    ...
    const save = (e) => {
        e.preventDefault();
        if (!model.id || !model.name || !model.email || !model.comment) {
            notification.error({
        ...
        setLoader(true)
        console.log(model)
        const reference = push(ref(db, 'commentsUser', model.id))
        set(reference, model)
            .then((res) => {
                console.log(res)
            ...
            {loader ? (
                ...
                <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
                    <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
                        <h2 className="text-3xl font-semibold text-center text-gray-700 mb-6">Comments</h2>
