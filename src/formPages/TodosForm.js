...
export default function TodosForm() {
    ...
    const save = (e) => {
        e.preventDefault();
        if(!model.id && !model.title && !model.complete){
            notification.error({
        ...
        setLoader(true)
        console.log(model)
        const reference = push(ref(db, 'todosUser', model.id))
        set(reference, model)
            .then((res) => {
                console.log(res)
            ...
            {loader ? (
                ...
                <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
                    <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
                        <h2 className="text-3xl font-semibold text-center text-gray-700 mb-6">Tods</h2>
                        ...
                        <div className="mb-4">
                            <Input
                                ...
                                className="border-2 border-gray-300 rounded-lg p-4 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                onChange={(e) => setModel({ ...model, complete: e.target.value })}
                            />
                        </div>
