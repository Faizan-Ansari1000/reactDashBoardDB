...
export default function Users() {
    ...
    useEffect(() => {
        const saveData = JSON.parse(localStorage.getItem("signUpUsers")) || [];
        ...
        if (location.state?.data) {
            ...
            if (!existData) {
                ...
                localStorage.setItem("signUpUsers", JSON.stringify(saveData));
            }
        }
            ...
            <div className="border rounded-lg mb-4">
                <header className="bg-black text-white flex justify-between items-center p-4 border rounded-md">
                    ...
                    <div className="text-end">
                        <Link to={'/DashBoard/SignUp'} className="text-white hover:text-cyan-400 text-sm sm:text-base md:text-lg">SignUp</Link>
                    </div>
                </header>
            </div>
