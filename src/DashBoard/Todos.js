...
export default function Todos() {
    ...
    useEffect(() => {
        const dataSaved = JSON.parse(localStorage.getItem("todosUSER")) || [];
        ...
        if (location.state?.data) {
            ...
            if (!existData) {
                ...
                localStorage.setItem("todosUSER", JSON.stringify(dataSaved));
            }
        }
            ...
            <div className="border rounded-lg mb-4">
                <header className="bg-black text-white flex justify-between items-center p-4 rounded-md">
                    ...
                    <div className="text-end">
                        <Link to={'/DashBoard/TodosForm'} className="text-white hover:text-cyan-400 text-sm sm:text-base md:text-lg">Feed</Link>
                    </div>
                </header>
            </div>
                ...
                <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                    <thead className="bg-cyan-800 text-white">
                        <tr>
                            ...
                            <th className="py-3 px-4 text-left font-semibold text-sm sm:text-base md:text-lg">True or False</th>
                    ...
                    <tbody className="text-gray-700">
                        {user.map((x, i) => (
                            <tr key={i} className={`hover:bg-gray-100 ${i % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
                                ...
                                <td className="py-3 px-4 text-sm sm:text-base md:text-lg">{x.complete}</td>
